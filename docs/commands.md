...
| `create_primitive` | Single primitive params or `items=[]`; supported `type`: `beveled_box`, `box`, `cube`, `panel`, `glass_panel`, `cylinder`, `cone`, `sphere`, `plane`, `label`; common params: `name`, `location`, `rotation`, `color`, `material_name`, `dimensions`, `bevel`, `radius`, `depth`, `size`, `text` | `{ok, message, objects, count}` |
| `create_furniture_preset` | Single furniture params or `items=[]`; supported `preset`: `shelf`, `cabinet`, `desk`, `bed`, `door`, `window`, `wall_art`; common params: `name`, `location`, `width`, `depth`, `height`, `thickness`, `wood_color`, `accent_color`, `fabric_color`, `glass_color` | `{ok, message, objects, count}` |
| `create_architecture_preset` | Single architecture params or `items=[]`; supported `preset`: `wall_opening`, `floor_tiles`, `ceiling_panels`, `stairs`, `railing`, `facade`; common params: `name`, `location`, `width`, `depth`, `height`, `thickness`, `wall_color`, `trim_color`, `floor_color`, `glass_color`, `accent_color` | `{ok, message, objects, count}` |
...
| `import_asset_from_library` | `id`, `name`, or `query` required; `location=[0,0,0]`, optional `rotation`, optional `scale`, `fit_to_bounds=true`, optional `target_size`, `target_location`, `align_to_floor=true` | `{ok, message, asset, imported, fit}` |
